import React, { useEffect, useMemo, useRef, useState } from "react";
import { useSearchParams, useNavigate } from "react-router-dom";
import Chart from "chart.js/auto";

const TOYOTA_RED = "#EB0A1E";

type LeaseResponse = {
  meta: { notes?: string[] };
  chartjs: {
    labels: string[];
    datasets: { label: string; type: string; stack?: string; data: number[] }[];
  };
  timeseries: {
    cumulative_finance: number[];
    cumulative_total_paid: number[];
    payment_total_per_month: number[];
  };
  totals: {
    vehicle_amount: number;
    term_months: number;
    residual_rate: number;
    residual_value: number;
    money_factor: number;
    apr_percent_est: number;
    acquisition_fee_financed: number;
    monthly_depreciation: number;
    monthly_finance: number;
    monthly_payment_total: number;
    total_depreciation: number;
    total_finance: number;
    total_paid: number;
  };
  schedule: {
    period: number;
    depreciation: number;
    finance: number;
    payment_total: number;
    residual_value_end: number;
  }[];
};

export default function LeaseQuotation() {
  const [sp] = useSearchParams();
  const [data, setData] = useState<LeaseResponse | null>(null);
  const [error, setError] = useState<string | null>(null);
  const navigate = useNavigate();

  const vehicleName = sp.get("vehicle_name") || "Toyota • Lease Visualization";
  const payload = useMemo(() => {
    const vehicle_amount = Number(sp.get("vehicle_amount") || "27500");
    const term_months = Number(sp.get("term_months") || "36");
    const money_factor = sp.get("money_factor");
    const acquisition_fee = sp.get("acquisition_fee");
    const body: Record<string, number> = { vehicle_amount, term_months } as any;
    if (money_factor) body.money_factor = Number(money_factor);
    if (acquisition_fee) body.acquisition_fee = Number(acquisition_fee);
    return body;
  }, [sp]);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        setError(null);
        let res = await fetch(
          "http://machine-virat.eastus2.cloudapp.azure.com:5000/lease/calcular",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          }
        );
        if (res.status === 404) {
          // Fallback: some backends may not have /lease/calcular deployed yet.
          // Try loan endpoint with neutral values and adapt the response to lease-like UI.
          const altBody = {
            vehicle_amount: payload.vehicle_amount,
            down_payment_cash: 0,
            term_months: payload.term_months,
            apr_percent: 0,
            tax_rate: 0.0,
          };
          const loanRes = await fetch(
            "http://machine-virat.eastus2.cloudapp.azure.com:5000/loan/Calculator",
            { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(altBody) }
          );
          if (!loanRes.ok) throw new Error(`HTTP ${loanRes.status}`);
          const loanJson: any = await loanRes.json();
          const adapted: LeaseResponse = {
            meta: { notes: ["Lease API not available, adapted from loan calculator (no tax)", ...(loanJson?.meta?.notes || [])] },
            chartjs: {
              labels: loanJson.chartjs.labels,
              datasets: [
                { label: "Depreciation", type: "bar", stack: "lease", data: loanJson.chartjs.datasets?.[0]?.data || [] },
                { label: "Finance (Rent)", type: "bar", stack: "lease", data: loanJson.chartjs.datasets?.[1]?.data || [] },
              ],
            },
            timeseries: {
              cumulative_finance: loanJson.timeseries.cumulative_interest,
              cumulative_total_paid: loanJson.timeseries.cumulative_total_paid,
              payment_total_per_month: loanJson.timeseries.payment_total_per_month,
            },
            totals: {
              vehicle_amount: loanJson.totals.vehicle_amount ?? payload.vehicle_amount,
              term_months: loanJson.totals.term_months ?? payload.term_months,
              residual_rate: 0,
              residual_value: 0,
              money_factor: 0,
              apr_percent_est: 0,
              acquisition_fee_financed: 0,
              monthly_depreciation: loanJson.chartjs.datasets?.[0]?.data?.[0] ?? 0,
              monthly_finance: loanJson.chartjs.datasets?.[1]?.data?.[0] ?? 0,
              monthly_payment_total: loanJson.totals.monthly_payment_total ?? 0,
              total_depreciation: 0,
              total_finance: loanJson.totals.total_interest ?? 0,
              total_paid: loanJson.totals.total_paid_including_tax ?? 0,
            },
            schedule: (loanJson.schedule || []).map((r: any) => ({
              period: r.period,
              depreciation: Number(r.principal || 0),
              finance: Number(r.interest || 0),
              payment_total: Number(r.payment_total || 0),
              residual_value_end: 0,
            })),
          };
          if (!cancelled) setData(adapted);
          return;
        }
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json: LeaseResponse = await res.json();
        if (!cancelled) setData(json);
      } catch (e: any) {
        if (!cancelled) setError(e?.message || "Failed to load lease quote");
      }
    })();
    return () => { cancelled = true; };
  }, [payload]);

  useEffect(() => { if (vehicleName) document.title = vehicleName; }, [vehicleName]);

  return (
    <div>
      <style>{CSS}</style>

      <header>
        <div className="wrap">
          <div className="brand">
            <img alt="Toyota logo" src="data:image/png;base64," />
            <h1>{vehicleName}</h1>
          </div>
        </div>
      </header>

      <div className="wrap" style={{ paddingTop: 22 }}>
        <div className="toolbar" style={{ marginBottom: 12 }}>
          <button className="btn primary" id="btnFocusBar" onClick={() => pulseById("stacked-title")}>
            Focus: Stacked Payments
          </button>
          <button className="btn" id="btnFocusLine" onClick={() => pulseById("trend-title")}>
            Focus: Cumulative
          </button>
          <button className="btn" id="btnDownload" onClick={() => downloadJSON(data)}>
            Download JSON
          </button>
          <button className="btn" onClick={() => navigate("/?mode=results")}>
            Back
          </button>
          <button
            className="btn"
            style={{ background: TOYOTA_RED, color: "white", marginLeft: "auto" }}
            onClick={() => {
              const params = new URLSearchParams({
                vehicle_name: vehicleName,
                monthly_payment: String(data?.totals.monthly_payment_total || 0),
                down_payment: String(data?.totals.acquisition_fee_financed || 0),
                type: "lease",
              });
              navigate(`/payment?${params.toString()}`);
            }}
          >
            Proceed with Payment
          </button>
        </div>

        {error && (
          <div className="card pad" style={{ borderColor: "#ffd9de", background: "#fff7f7" }}>
            <h3>Could not load lease quote</h3>
            <p style={{ color: "#a30717" }}>{String(error)}</p>
            <p className="fine" style={{ marginTop: 6 }}>Check CORS or try again.</p>
          </div>
        )}

        {!data ? (
          <div className="card pad">
            <h3>Loading…</h3>
            <p className="fine">Fetching lease quote from API with your parameters.</p>
          </div>
        ) : (
          <Content data={data} vehicleName={vehicleName} />
        )}
      </div>
    </div>
  );
}

function Content({ data, vehicleName }: { data: LeaseResponse; vehicleName: string }) {
  const barRef = useRef<HTMLCanvasElement | null>(null);
  const lineRef = useRef<HTMLCanvasElement | null>(null);
  const didFreezeRef = useRef(false);
  const destroyedByFreezeRef = useRef(false);
  const hasResidual = useMemo(() => (data.schedule || []).some(r => Number(r.residual_value_end) > 0), [data.schedule]);
  const totals = data.totals;
  const computed = useMemo(() => {
    const n = Math.max(1, Number(totals.term_months || 0));
    const totalDep = Number(totals.monthly_depreciation || 0) * n;
    const totalFin = Number(totals.monthly_finance || 0) * n;
    const percentMsrpPaid = totals.vehicle_amount ? (Number(totals.total_paid || 0) / Number(totals.vehicle_amount)) * 100 : 0;
    const firstYearOutlay = Number(totals.monthly_payment_total || 0) * Math.min(12, n);
    const dailyCost = (Number(totals.monthly_payment_total || 0) * 12) / 365;
    const costPerThousand = totals.vehicle_amount ? Number(totals.total_paid || 0) / (Number(totals.vehicle_amount) / 1000) : 0;
    const depShare = (totalDep && (totalDep + totalFin)) ? (totalDep / (totalDep + totalFin)) * 100 : 0;
    const finShare = (totalDep && (totalDep + totalFin)) ? (totalFin / (totalDep + totalFin)) * 100 : 0;
    return { n, totalDep, totalFin, percentMsrpPaid, firstYearOutlay, dailyCost, costPerThousand, depShare, finShare };
  }, [totals]);

  useEffect(() => {
    const barCtx = barRef.current?.getContext("2d");
    const _bar = barCtx
      ? new Chart(barCtx, {
          type: "bar",
          data: {
            labels: data.chartjs.labels,
            datasets: (data.chartjs.datasets.map((ds) => ({
              ...ds,
              backgroundColor:
                ds.label === "Depreciation" ? "#2e7d32" : "#9e9e9e",
              borderWidth: 0,
              borderRadius: 6,
              barPercentage: 0.9,
              categoryPercentage: 1.0,
            })) as any),
          },
          options: {
            animation: false,
            responsive: true,
            maintainAspectRatio: false,
            plugins: {
              legend: { position: "top" },
              tooltip: { callbacks: { label: (c) => `${c.dataset.label}: ${currency(Number(c.parsed.y))}` } },
            },
            interaction: { mode: "index", intersect: false },
            scales: {
              x: { stacked: true, grid: { display: false } },
              y: { stacked: true, beginAtZero: true, ticks: { callback: (v) => currency(Number(v)) } },
            },
          },
        })
      : null;

    const lineCtx = lineRef.current?.getContext("2d");
    const _line = lineCtx
      ? new Chart(lineCtx, {
          type: "line",
          data: {
            labels: data.chartjs.labels,
            datasets: [
              { label: "Cumulative Finance", data: data.timeseries.cumulative_finance, borderWidth: 2, fill: false, tension: 0.25 },
              { label: "Cumulative Total Paid", data: data.timeseries.cumulative_total_paid, borderWidth: 2, fill: false, tension: 0.25 },
            ],
          },
          options: {
            animation: false,
            responsive: true,
            maintainAspectRatio: false,
            plugins: { legend: { position: "top" }, tooltip: { callbacks: { label: (c) => `${c.dataset.label}: ${currency(Number(c.parsed.y))}` } } },
            scales: { x: { grid: { display: false } }, y: { beginAtZero: true, ticks: { callback: (v) => currency(Number(v)) } } },
          },
        })
      : null;

    if (!didFreezeRef.current) {
      requestAnimationFrame(() => {
        if (_bar) freezeChartToImage(_bar, "stackedChart");
        if (_line) freezeChartToImage(_line, "trendChart");
        didFreezeRef.current = true;
        destroyedByFreezeRef.current = true;
      });
    }

    return () => {
      if (!destroyedByFreezeRef.current) {
        _bar?.destroy();
        _line?.destroy();
      }
    };
  }, [data]);

  return (
    <div className="grid">
      <section className="card chart-card" aria-labelledby="stacked-title">
        <h3 id="stacked-title">Lease Breakdown (Depreciation • Finance)</h3>
        <div style={{ position: "relative", width: "100%", height: 420 }}>
          <canvas id="stackedChart" ref={barRef} aria-label="Stacked bar chart of lease payments" />
        </div>
        <div className="chart-subtitle">Monthly lease components across the term. Static, no animations.</div>
      </section>

      <aside className="card pad">
        <h3>Summary</h3>
        <div className="kpis">
          {(
            [
              ["Vehicle", vehicleName, true],
              ["Vehicle Price", currency(totals.vehicle_amount), totals.vehicle_amount > 0],
              ["Monthly Payment", currency(totals.monthly_payment_total), totals.monthly_payment_total > 0],
              ["Lease Term", `${totals.term_months} months`, totals.term_months > 0],
              ["Residual Value", currency(totals.residual_value), hasResidual && totals.residual_value > 0],
              ["Residual Rate", `${(totals.residual_rate * 100).toFixed(1)}%`, hasResidual && totals.residual_rate > 0],
              ["Total Lease Cost", currency(totals.total_paid), totals.total_paid > 0],
              ["Monthly Depreciation", currency(totals.monthly_depreciation), totals.monthly_depreciation > 0],
              ["Monthly Finance Charge", currency(totals.monthly_finance), totals.monthly_finance > 0],
              ["Effective Interest Rate", `${totals.apr_percent_est.toFixed(1)}%`, totals.apr_percent_est > 0],
              ["Acquisition Fee", currency(totals.acquisition_fee_financed), totals.acquisition_fee_financed > 0],
              // New, customer-friendly derived KPIs
              ["% of MSRP Paid", `${Math.round(computed.percentMsrpPaid)}%`, computed.percentMsrpPaid > 0],
              ["First-Year Outlay", currency(computed.firstYearOutlay), computed.firstYearOutlay > 0],
              ["Daily Cost", currency(computed.dailyCost), computed.dailyCost > 0],
              ["Cost per $1k MSRP", currency(computed.costPerThousand), computed.costPerThousand > 0],
              ["Payment Mix: Depreciation", `${Math.round(computed.depShare)}%`, computed.depShare > 0],
              ["Payment Mix: Finance", `${Math.round(computed.finShare)}%`, computed.finShare > 0],
              ["Flexibility Score", `${Math.round((totals.term_months / 60) * 100)}%`, totals.term_months > 0],
              ["Upgrade Opportunity", `${totals.term_months} months`, totals.term_months > 0],
            ] as const
          )
            .filter(([, , show]) => !!show)
            .map(([label, val]) => (
              <div className="kpi" key={label as string}>
                <div className="label">{label as string}</div>
                <div className="value">{val as string}</div>
              </div>
            ))}
        </div>

        <div className="note" style={{ marginTop: 12 }}>
          <strong>Note:&nbsp;</strong>
          <div>
            {(data.meta?.notes || []).map((n, i) => (
              <p key={i} style={{ margin: "4px 0" }}>• {n}</p>
            ))}
          </div>
        </div>
      </aside>

      <section className="card chart-card" aria-labelledby="trend-title">
        <h3 id="trend-title">Cumulative Finance & Total Paid Over Time</h3>
        <div style={{ position: "relative", width: "100%", height: 420 }}>
          <canvas id="trendChart" ref={lineRef} aria-label="Line chart of cumulative totals" />
        </div>
        <div className="chart-subtitle">Track cumulative finance and outlay growth month over month.</div>
      </section>

      <section className="card pad" aria-labelledby="schedule-title">
        <h3 id="schedule-title">Lease Schedule (Monthly)</h3>
        <div style={{ maxHeight: 360, overflow: "auto", border: "1px solid var(--border)", borderRadius: 12 }}>
          <table>
            <thead>
              <tr>
                <th>Month</th>
                <th>Depreciation</th>
                <th>Finance</th>
                <th>Total</th>
                {hasResidual && <th>Residual End</th>}
              </tr>
            </thead>
            <tbody>
              {data.schedule.map((r) => (
                <tr key={r.period}>
                  <td>{r.period}</td>
                  <td>{currency(Number(r.depreciation))}</td>
                  <td>{currency(Number(r.finance))}</td>
                  <td>{currency(Number(r.payment_total))}</td>
                  {hasResidual && <td>{currency(Number(r.residual_value_end))}</td>}
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </section>

      <footer className="wrap" style={{ paddingLeft: 0, paddingRight: 0 }}>
        <p className="fine">Demo lease view without sales tax; dealers may apply taxes differently.</p>
      </footer>
    </div>
  );
}

function currency(n: number) {
  return new Intl.NumberFormat(undefined, { style: "currency", currency: "USD" }).format(n);
}

function pulseById(id: string) {
  const el = document.getElementById(id)?.closest(".card");
  if (!el) return;
  el.classList.add("pulse");
  setTimeout(() => el.classList.remove("pulse"), 900);
}

function downloadJSON(d: LeaseResponse | null) {
  if (!d) return;
  const raw = JSON.stringify(d, null, 2);
  const blob = new Blob([raw], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = Object.assign(document.createElement("a"), { href: url, download: "toyota_lease_demo.json" });
  document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
}

function freezeChartToImage(chart: Chart | null, canvasId: string) {
  if (!chart) return;
  const canvas = document.getElementById(canvasId) as HTMLCanvasElement | null;
  if (!canvas) return;
  const url = chart.toBase64Image("image/png", 1);
  const img = new Image();
  img.src = url; img.alt = canvas.getAttribute("aria-label") || "chart";
  img.style.width = "100%"; img.style.height = "420px";
  canvas.replaceWith(img);
  chart.destroy();
}

const CSS = `
:root{
  --toyota-red:#EB0A1E; --bg:#ffffff; --text:#0f0f0f; --muted:#f4f4f4; --ink-2:#666; --ink-3:#999; --border:#e8e8e8; --card-shadow: 0 8px 24px rgba(0,0,0,.06), 0 2px 8px rgba(0,0,0,.04); --radius: 18px;
}
*{box-sizing:border-box}
html{scroll-behavior:auto}
body{overscroll-behavior:contain}
@keyframes pulse{0%{box-shadow:0 0 0 0 rgba(235,10,30,.35)}100%{box-shadow:0 0 0 6px rgba(235,10,30,0)}}
.pulse{animation:pulse .9s ease-out 1}
html,body{margin:0;height:100%;background:var(--bg);color:var(--text);font-family:Poppins,system-ui,-apple-system,Segoe UI,Roboto,Arial,"Noto Sans",sans-serif}
a{color:var(--toyota-red);text-decoration:none}
header{background:linear-gradient(0deg, rgba(235,10,30,.96), rgba(235,10,30,.96)), url(''); color:#fff;border-bottom:1px solid rgba(255,255,255,.08)}
.wrap{max-width:1200px;margin:0 auto;padding:18px}
.brand{display:flex;align-items:center;gap:14px}
.brand img{height:36px;filter:drop-shadow(0 1px 0 rgba(0,0,0,.15))}
.brand h1{font-size:18px;margin:0;font-weight:600;letter-spacing:.3px}
.grid{display:grid;gap:22px;grid-template-columns:1.2fr .8fr}
@media (max-width: 980px){ .grid{grid-template-columns:1fr} }
.card{background:#fff;border:1px solid var(--border);border-radius:var(--radius);box-shadow:var(--card-shadow)}
.card h3{margin:0 0 8px;font-size:16px}
.card .pad{padding:16px 18px}
.chart-card{padding:14px 16px 18px}
canvas{width:100%;height:420px;}
.chart-subtitle{font-size:12px;color:var(--ink-2);margin-top:6px}
.kpis{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}
@media (max-width: 560px){ .kpis{grid-template-columns:1fr} }
.kpi{padding:14px 16px;border-radius:14px;border:1px solid var(--border);background:var(--muted)}
.kpi .label{font-size:12px;color:var(--ink-3)}
.kpi .value{font-size:18px;font-weight:600}
.note{display:flex;gap:10px;align-items:flex-start;padding:12px 14px;border-radius:12px;background:#fff7f7;border:1px solid #ffd9de}
.pill{display:inline-block;padding:2px 8px;border-radius:999px;background:#ffe0e3;color:#a30717;font-weight:600;font-size:12px;letter-spacing:.2px}
table{width:100%;border-collapse:collapse}
th,td{padding:10px 8px;border-bottom:1px dashed var(--border);font-size:12px;text-align:right;white-space:nowrap}
th:first-child, td:first-child{text-align:left}
tbody tr:hover{background:#fafafa}
footer{margin-top:24px;border-top:1px solid var(--border)}
.fine{font-size:12px;color:var(--ink-3)}
.toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
.btn{appearance:none;border:1px solid var(--border);background:#fff;border-radius:12px;padding:8px 12px;font-weight:600;cursor:pointer}
.btn.primary{background:var(--toyota-red);border-color:var(--toyota-red);color:#fff}
`;


